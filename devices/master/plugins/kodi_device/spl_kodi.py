...
# usefull link: https://www.loxwiki.eu/plugins/servlet/mobile?contentId=60556203#content/view/60556203
    ...
    def play_media(self, movie_url, movie_mime_type, current_time=0):
        ...
        if url_st.query and url_st.scheme == "http":
        ...
        try:
            ...
            self.cast_info['play'] = defaults.PLAYER_STATE_EMPTY
        if current_time:
            # in case we have a start position, we need to give the player some time to be sure that update_ status returns data which match to the stream
            time.sleep(3)
        self.update_status()
    ...
    def update_status(self):
        ...
        response = self.doJsonRPC(payload)
        try:
            self.cast_info['duration'] = self.time_to_timestamp(
                response['result']['totaltime'])
            self.cast_info['current_time'] = self.time_to_timestamp(
    ...
    def doJsonRPC(self, payload):
        try:
            ...
            print('Kodi jsonRPC exception on {0}: {1}'.format(
                self.host, str(ex)))
            self.online = False
            return None
...
class SplPlugin(SplThread):
    ...
    def __init__(self, modref):
        ...
        self.config = JsonStorage(
            self.plugin_id, 'backup', "config.json", {'stopdelay': 0.1})
    ...
    def event_listener(self, queue_event):
        ...
        if queue_event.type == defaults.DEVICE_PLAY_STOP:
            ...
            if cast and cast.online:
                ...
                if not cast:
                    print("kodi player stop command: Device {0} not found".format(
                        queue_event.data['device_friendly_name']))
                else:
                    print("kodi player stop command: Device {0} not online".format(
                        queue_event.data['device_friendly_name']))
    ...
    def send_device_play_status(self, device_friendly_name, state_change_flag):
        if device_friendly_name in self.devices:
            ...
            # state change is set either on request or of kodi itself is asking for
            cast_info['state_change'] = cast_info['state_change'] or state_change_flag
